<template>
  <ul class="pagination m-t-0">
    <li :class="{'disabled': isFirstPage}" @click="turnPage(-1)">
      <a href="javascript:;">
        <i class="fa fa-arrow-left"></i>
      </a>
    </li>
    <li v-for="i in displayPageBtns" track-by="$index" :class="{'active': i === curPageIdx + 1}">
      <a href="javascript:;" v-if="i" @click="handleClick(i)">
        {{i}}
      </a>
      <a v-else>...</a>
    </li>
...
export default {
  ...
  methods: {
    ...
    turnPage (i) {
      // 设置class为disabled并不能阻止@click, 此处需要再次验证
