<template>
  <div>
    <div class="row">
      ...
      <div class="col-sm-6 col-md-7 col-lg-8 clearfix" v-if="$root.userData">
        <a v-link="`/msg/add`" class="btn btn-default btn-sm pull-right">
          <span class="fa-stack m-r-5">
            <i class="fa fa-commont-o fa-stack-2x"></i>
    ...
    <div class="row min-h-180">
      <div class="col-sm-12">
        <ul class="list-group m-t-5">
          <li class="list-group-item clearfix" v-for="msg in msgs" track-by="id">
            <h4 class="list-group-item-heading">
              <a v-link="`/msg/detail/${msg.id}`">{{msg.title | cutLongText}}</a>
              <smal class="italic">by <a v-link="{path: '/msg', query: {authors: msg.author}}">{{msg.author}}</a></smal>
              <span class="badge pull-right">{{msg.content | dateTimeFormatter}}</span>
            </h4>
            <smal class="list-group-item-text">{{msg.content | cutLongText 20}}</smal>
        ...
        <h4 class="text-muted text-center line-h-150 italic" v-show="!total">(暂无留言信息)</h4>
      </div>
    </div>
