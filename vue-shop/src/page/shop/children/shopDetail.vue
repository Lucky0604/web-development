<template>
	<div class="rating_page">
		<head-top head-title="Shop Detail" go-back="true"></head-top>
		<section class="activities_container">
			...
			<ul class="activities_ul">
				<li v-for="item in shopDetail.activities" :key="item.id">
					...
					<span>{{item.description}}(Only in App)</span>
				</li>
			</ul>
			<ul class="activities_ul">
		...
		<section class="shop_status_container">
			<router-link class="shop_status_header" to="/shop/shopDetail/shopSafe">
				<span class="shop_detail_title">Identification</span>
				<div>
					<span class="identification_detail">Identification detail</span>
			...
			<section class="shop_status_detail">
				...
				<div class="check_date">
					<p>
						<span>Result:</span>
						<span class="shop_status_well" v-if="shopDetail.status == 1">Good</span>
						<span class="shop_status_bad" v-else>Bad</span>
					</p>
					<p>
						<span>Date:</span>
		...
		<section class="shop_status_info">
			<header>Shop Info</header>
			<p>{{shopDetail.name}}</p>
			<p>Address: {{shopDetail.address}}</p>
			<p>Opening Hours: [{{shopDetail.opening_hours[0]}}]</p>
			<p @click="showLicenseImg(shopDetail.license.business_license_image)">
				<span>Business Image</span>
				<svg class="description_arrow" width="14" height="14" xmlns="http://www.w3.org/2000/svg">
					<path d="M0 0 L8 7 L0 14" stroke="#bbb" stroke-width="1.5" fill="none" />
				</svg>
			</p>
			<p @click="showLicenseImg(shopDetail.license.catering_service_license_image)">
				<span>Catering Service License Image</span>
				<svg class="description_arrow" width="14" height="14" xmlns="http://www.w3.org/2000/svg">
					<path d="M0 0 L8 7 L0 14" stroke="#bbb" stroke-width="1.5" fill="none" />
				</svg>
			</p>
		...
		<transition name="fade">
			<section class="license_container" v-if="showlicenseImg" @click="showlicenseImg = false">
				<img :src="getImgPath(licenseImg)" alt="">
			</section>
		</transition>
