...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 移出购物车，所需７个参数：商铺id, 食品分类id, 食品id, 食品规格id, 食品名字，食品价格，食品规格
      removeOutCart(category_id, item_id, food_id, name, price, specs) {
        this.REDUCE_CART({shopid: this.shopId, category_id, item_id, food_id, name, price, specs});
      },

      /**
       * 初始化和shopCart变化时，重新获取购物车改变过的数据，赋值categoryNum, totalPrice, cartFoodList, 整个数据流自上而下的形式
      ...
      // control the cart list show or hide
      toggleCartList() {
      ...
      // listen the point if it get into cart
      listenInCart() {
        if (!this.receiveInCart) {
          this.receiveInCart = true;
          this.$refs.cartContainer.addEventListener('animationed', () => {
      ...
      // get different types comments list
      ...
      // record the current choosed spec's index
      chooseSpecs (index) {
        this.specsIndex = index;
      },

      // add multiple specs foods to the cart
      addSpecs(category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock) {
        this.ADD_CART({shopid: this.shopId, category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock});
        this.showChooseList();
      },

      // show the tips, can't remove the item
      showReduceTip() {
    ...
    watch: {
      ...
      // when the cart list changed, there's no shop, then hide
      ...
      // toggle state about items and comments
