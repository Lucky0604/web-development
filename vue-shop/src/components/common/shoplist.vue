...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async listenPropChange () {
        ...
        if (process.env.NODE_ENV !== 'development') {
          this.shopListArr = this.shopListAr.reverse();
        }
      },
      // 开发环境和编译环境loading隐藏方式不同
      hideLoading() {
        if (process.env.NODE_ENV !== 'development') {
    ...
    watch: {
      // 监听父级传来的restaurantCategoryIds, 当值发生变化的时候重新获取餐厅数据，作用于排序和筛选
      ...
      // 监听父级的确认按钮是否被点击，并且返回一个自定义时间通知父级，已经接收到数据，此时父级才可以清除已选状态
