/**
 * @Author: lucky
 * @Date:   2017-04-29T19:18:23+08:00
 * @Last modified by:   lucky
 * @Last modified time: 2017-04-29T22:12:05+08:00
 */
...
const addMethods = (_this) => {
  _this.find = async function (req, res, next) {
    return $.result(res, await _this.model.find({'_id': req.params._id}))
  }

  _this.all = async function (req, res, next) {
    return $.result(res, await _this.model.all({}, req.query.start))
  }

  _this.create = async function (req, res, next) {
    let documents = await _this.model.create(req.body)
    if (documents === -1) {
      $.result(res, 'create failed')
    } else {
